...
func TestTimestamp_Value(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid vdb.Timestamp",
			ts:   vdb.Timestamp{time.Date(2024, 1, 15, 19, 5, 0, 0, time.UTC)},
			want: "2024-01-15T19:05:00Z",
		},
		{
			name: "zero time",
			ts:   vdb.Timestamp{time.Time{}},
			want: nil,
		},
	}
...
func TestTimestamp_Integration(t *testing.T) {
	...
	t.Run("insert vdb.Timestamp", func(t *testing.T) {
		now := vdb.Timestamp{time.Date(2024, 1, 15, 19, 5, 0, 0, time.UTC)}
	...
	t.Run("scan vdb.Timestamp", func(t *testing.T) {
		var ts vdb.Timestamp
	...
	t.Run("round trip", func(t *testing.T) {
		original := vdb.Timestamp{time.Now().UTC().Truncate(time.Second)}
